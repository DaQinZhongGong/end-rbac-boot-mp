...
/**
 * RedisUtils类是一个封装了Redis操作的工具类，提供了丰富的Redis操作方法，包括缓存失效时间的设置与获取、键的查找与删除、String和Map类型数据的操作等。通过该类，可以方便地在Java应用中进行Redis操作，提高开发效率。
 *
 * @author free
 */
@Component
// 在Java中，@SuppressWarnings注解是用来告诉编译器忽略特定的警告信息的。这个注解可以应用于类、字段、方法或参数级别。它接受一个字符串数组作为参数，每个字符串指定了要忽略的警告类型。
// - "unchecked" 用于抑制未经检查的转换警告，通常出现在泛型操作中。
// - "all" 用于抑制所有警告，这在某些情况下很有用，但应谨慎使用以避免隐藏潜在的问题。
