...
/**
 * 用于管理 Spring 应用上下文（ApplicationContext）。它实现了 ApplicationContextAware 和 DisposableBean 接口，以便在 Spring 容器初始化和销毁时执行相应操作。主要功能包括：
 * <p>
 * 1. 获取 Spring 容器中的 Bean。
 * 2. 获取 Spring Boot 配置信息。
 * 3. 提供回调机制，在容器初始化后执行回调函数。
 * 4. 清除应用上下文。
 *
 * @author free
 */
