 ...
 * methods.
 * * 支持匿名访问  PutMapping
 * AnonymousPutMapping是一个结合了Spring MVC的@RequestMapping注解和自定义的@AnonymousAccess注解的自定义注解。它用于将HTTP的PUT请求映射到支持匿名访问的处理器方法上，从而方便地创建RESTful API端点，允许匿名用户更新资源。
 *
 * @author free
...
@Documented
// @RequestMapping(method = RequestMethod.PUT)：这是Spring MVC的@RequestMapping注解，用于将HTTP请求映射到处理器方法上。这里指定了method = RequestMethod.PUT，表示该方法处理PUT请求。
